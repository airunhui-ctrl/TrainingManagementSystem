...
describe('P1 非支付写操作闭环', () => {
  ...
  test('开票通过/驳回、重新申请和重复处理均受状态门禁保护', async () => {
    ...
    const retry = await request(`/invoices/${rejectedInvoice.data.id}/reapply`, { method: 'POST', body: JSON.stringify({ title: 'P1 可重申请企业（修订）', taxNo: '91350000P1TEST02', email: 'p1-retry@example.com', orderIds: [] }) }, demoToken)
    expect(retry.response.status).toBe(201)
    expect(retry.data.id).not.toBe(rejectedInvoice.data.id)
    expect(retry.data.title).toBe('P1 可重申请企业（修订）')
    expect(retry.data.retryOfInvoiceId).toBe(rejectedInvoice.data.id)
    expect(retry.data.status).toBe('待处理')
    const retryRejected = await request(`/admin/invoices/${retry.data.id}/process`, { method: 'POST', body: JSON.stringify({ approved: false, rejectReason: '税号仍需核对' }) }, adminToken)
    expect(retryRejected.response.status).toBe(201)
    expect(retryRejected.data.status).toBe('已驳回')
    expect(retryRejected.data.rejectReason).toBe('税号仍需核对')
    const finalRetry = await request(`/invoices/${retry.data.id}/reapply`, { method: 'POST', body: JSON.stringify({ title: 'P1 可重申请企业（最终修订）', taxNo: '91350000P1TEST02', email: 'p1-retry@example.com', orderIds: [] }) }, demoToken)
    expect(finalRetry.response.status).toBe(201)
    expect(finalRetry.data.retryOfInvoiceId).toBe(retry.data.id)
    const finalApproved = await request(`/admin/invoices/${finalRetry.data.id}/process`, { method: 'POST', body: JSON.stringify({ approved: true, invoiceNo: 'P1-INV-RETRY-OK' }) }, adminToken)
    expect(finalApproved.response.status).toBe(201)
    expect(finalApproved.data.status).toBe('已开票')
