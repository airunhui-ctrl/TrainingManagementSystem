...
test('旧 AppState/文本日期数据库可无损迁移到 Prisma 结构化表', async () => {
  ...
  for (const user of [['u-demo','demo','user'],['u-admin','admin','admin'],['u-operator','operator','operator']]) legacy.prepare('INSERT INTO User VALUES(?,?,?,?,?,?,?,?,?,?)').run(user[0],user[1],hashPassword('123456'),user[2],user[1],'测试企业',1,0,timestamp,timestamp)
  const state = {
