...
describe('生产化 API 闭环', () => {
  ...
  test('报名重复校验、在线支付、开票处理、退款和分页筛选通过接口闭环', async () => {
    ...
    expect((await request(`/orders/${created.data.id}/pay`, { method: 'POST', body: JSON.stringify({ method: 'online', channel: 'wechat' }) }, demoToken)).data.status).toBe('已支付')
    const invoice = await request('/invoices', { method: 'POST', body: JSON.stringify({ title: '接口企业', taxNo: '91350200API', email: 'api@example.com', orderIds: [created.data.id] }) }, demoToken)
    ...
    expect((await request(`/admin/orders/${created.data.id}/refund`, { method: 'POST' }, adminToken)).data.status).toBe('已取消')
