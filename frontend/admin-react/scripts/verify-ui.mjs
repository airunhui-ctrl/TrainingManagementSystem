...
check(!app.includes('TEMPLATE_FIELD_PAGE_SIZE') && !app.includes('fieldPage') && !app.includes('template-pagination'), '报名模板字段仍按分页展示，应改为全部字段平铺')
check(app.includes('template-required') && app.includes('template-remove') && app.includes('template-options') && app.indexOf('template-remove') > app.indexOf('template-required') && app.indexOf('template-options') > app.indexOf('template-remove'), '报名模板删除按钮未放在必填选项后面')
check(app.includes('removeTemplateField') && app.includes('确定删除字段') && app.includes('onFieldRemove') && css.includes('.template-remove-text') && css.includes('white-space:nowrap'), '报名模板字段删除未接二次确认或未改为水平红色文字按钮')
check(app.includes('/admin/course-categories') && app.includes('课程分类<select') && !app.includes('课程分类<input'), '课程分类未使用数字字典下拉')
check(app.includes('templateLocked') && app.includes('locked={templateLocked}') && app.includes('该模板已关联报名中的课程'), '报名模板未接入已关联课程锁定提示')
...
check(css.includes('.course-modal > .modal-actions') && css.includes('.template-modal > .modal-actions') && css.includes('flex: 0 0 auto') && css.includes('.modal-scroll') && css.includes('.app-dialog-actions') && css.includes('flex: 0 0 auto'), '管理端弹窗操作栏未固定为独立底部栏')
check(css.includes('.modal-checkbox') && !/\.modal-checkbox\s*\{[^}]*border:\s*1px/.test(css), '可勾选项仍带外层边框文本框')
