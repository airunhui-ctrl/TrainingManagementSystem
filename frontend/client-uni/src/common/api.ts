...
export const api = {
  ...
  createOrder: (courseId: string, participants: Array<Record<string, string>>, paymentMethod: 'online' | 'offline' = 'online') => request<{ id: string; amount: number; originalAmount: number; discount: number; participantCount: number; status: string }>({ url: '/orders', method: 'POST', data: { courseId, participants: participants.map((data) => ({ data })), paymentMethod } }),
