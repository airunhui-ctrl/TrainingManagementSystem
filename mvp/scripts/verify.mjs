...
const checks = {
  ...
  apiHasMvpRoutes: ["'courses'", "'orders/quote'", "'invoices'", "'invoices/:id/reapply'", "'admin/dashboard'"].every((route) => mvpController.includes(route)),
  ...
  clientInvoiceReapplyUsesDedicatedEndpoint: clientApi.includes('reapplyInvoice:') && clientApi.includes("/invoices/${encodeURIComponent(invoiceId)}/reapply") && clientBusiness.includes('api.reapplyInvoice(') && clientBusiness.includes('retryOfInvoiceId'),
